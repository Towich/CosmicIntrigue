...
class VoteAdapter(var onVote:(Int?)->Unit, players: List<Player>, val userId : Long) :RecyclerView.Adapter<VoteAdapter.VoteViewHolder>(){
    ...
    init{
        ...
        players.forEach{
            Users.add(AdaptedPlayer(it.id?.toInt(),it.login,userId == it.id,false,-1))
        }
    }
